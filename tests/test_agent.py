...
class AgentTest(unittest.TestCase):
    ...
    def test_chat_stream_compacts_tool_output_only_for_model_context(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["output_artifact"], f".agent/artifacts/{payload['call_id']}.json")
            artifact = json.loads((root / payload["output_artifact"]).read_text(encoding="utf-8"))
