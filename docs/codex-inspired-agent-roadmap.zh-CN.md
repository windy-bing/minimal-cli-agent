...
- `tools/router.rs` 和 `tools/orchestrator.rs`：Codex 将工具可见性、审批、沙箱、重试和 dispatch trace 分层；本项目已具备 registry/pipeline/dispatch trace，下一步应补 sandbox attempt 语义。
...
- [ ] Sandbox attempt trace：把权限决策、沙箱策略、重试尝试和最终执行结果串成可查询 trace。
